...
% Last Modified by GUIDE v2.5 15-Oct-2012 00:27:40
...
rus='qwertyuiop[]\asdfghjkl;zxcvbnm,./`1234567890-=\"'; % скрипт для выделения ТОЛЬКО маршрутов в лабиринте (в виде непрерывной последовательности букв)
bukv(1)=' ';
for k=1:length(opit)
    opit(k)==rus;
    nu=sum(ans);
...
odnogram=0;
for x=1:length(probi)
    odnogram=odnogram+(length(probi{x})-1);
end
...
write=strvcat(write0,blanks(3)','Синтактика',blanks(1)', sintaktika1,blanks(1)',['Всего однограмм: ' num2str(odnogram)]);
...
ngram{1}=0;
ngram2{1}=0;
 proba1=[' ' proba1];                    % добавляем служебный символ в начало, чтобы было "-1" значение
for u=1:(length(proba_matrix)-(n-1))
    ...
    if isempty(poisk)==0
       ...
       while isempty(poisk)==0
           poisk(1)
       proba1=[proba1(1:poisk(1)-1) isk1 proba1((poisk(1)+ n):length(proba1))] % изменяется запись пробы в которой ищем, чтобы 2 раза нельзя было найти одну и ту же нграмму
       poisk=strfind(proba1,isk)
       end
    end
            end 
...
while 1<length(ngram)
if length(ngram)<=41
    write1=[ngram{2} ':  ' num2str(ngram{3}) '  (' sprintf('%1.2f',(ngram{3}/vse)) ')'];
    for y=4:2:length(ngram)
    write1=strvcat(write1,[ngram{y} ':  ' num2str(ngram{y+1}) '  (' sprintf('%1.2f',(ngram{y+1}/vse)) ')']);
    end
    break
elseif length(ngram)<=81
    write1=[ngram{2} ':  ' num2str(ngram{3}) '  (' sprintf('%1.2f',(ngram{3}/vse)) ')' blanks(8) ngram{42} ':  ' num2str(ngram{43}) '  (' sprintf('%1.2f',(ngram{43}/vse)) ')'];
    for y=4:2:40
        if y+41<=length(ngram)
    write1=strvcat(write1,[ngram{y} ':  ' num2str(ngram{y+1}) '  (' sprintf('%1.2f',(ngram{y+1}/vse)) ')' blanks(8) ngram{y+40} ':  ' num2str(ngram{y+41}) '  (' sprintf('%1.2f',(ngram{y+41}/vse)) ')' ]);
        else
    write1=strvcat(write1,[ngram{y} ':  ' num2str(ngram{y+1}) '  (' sprintf('%1.2f',(ngram{y+1}/vse)) ')' ]) ;   
        end
    end
    break
elseif length(ngram)<=121
    write1=[ngram{2} ':  ' num2str(ngram{3}) '  (' sprintf('%1.2f',(ngram{3}/vse)) ')' blanks(8) ngram{42} ':  ' num2str(ngram{43}) '  (' sprintf('%1.2f',(ngram{43}/vse)) ')'...
        blanks(8) ngram{82} ':  ' num2str(ngram{83}) '  (' sprintf('%1.2f',(ngram{83}/vse)) ')'];
    for y=4:2:40
        if y+81<=length(ngram)
    write1=strvcat(write1,[ngram{y} ':  ' num2str(ngram{y+1}) '  (' sprintf('%1.2f',(ngram{y+1}/vse)) ')' blanks(8) ngram{y+40} ':  ' num2str(ngram{y+41}) '  (' sprintf('%1.2f',(ngram{y+41}/vse)) ')'...
        blanks(8) ngram{y+80} ':  ' num2str(ngram{y+81}) '  (' sprintf('%1.2f',(ngram{y+81}/vse)) ')']);
        else
    write1=strvcat(write1,[ngram{y} ':  ' num2str(ngram{y+1}) '  (' sprintf('%1.2f',(ngram{y+1}/vse)) ')' blanks(8) ngram{y+40} ':  ' num2str(ngram{y+41}) '  (' sprintf('%1.2f',(ngram{y+41}/vse)) ')' ]) ;   
        end
    end
    ...
    write1=[ngram{2} ':  ' num2str(ngram{3}) '  (' sprintf('%1.2f',(ngram{3}/vse)) ')' blanks(8) ngram{42} ':  ' num2str(ngram{43}) '  (' sprintf('%1.2f',(ngram{43}/vse)) ')'...
        blanks(8) ngram{82} ':  ' num2str(ngram{83}) '  (' sprintf('%1.2f',(ngram{83}/vse)) ')'];
    for y=4:2:(length(ngram)-81)
        if y<=40
    write1=strvcat(write1,[ngram{y} ':  ' num2str(ngram{y+1}) '  (' sprintf('%1.2f',(ngram{y+1}/vse)) ')' blanks(8) ngram{y+40} ':  ' num2str(ngram{y+41}) '  (' sprintf('%1.2f',(ngram{y+41}/vse)) ')'...
        blanks(8) ngram{y+80} ':  ' num2str(ngram{y+81}) '  (' sprintf('%1.2f',(ngram{y+81}/vse)) ')']);
        else 
    write1=strvcat(write1,[ngram{y+80} ':  ' num2str(ngram{y+81}) '  (' sprintf('%1.2f',(ngram{y+81}/vse)) ')']);
        end
    end
    break
end
...
% --- Executes on button press in semant.
function semant_Callback(hObject, eventdata, handles)
clear global sem_zon
clear global bezpl
clear global probi
global opit
...
% hObject    handle to semant (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
